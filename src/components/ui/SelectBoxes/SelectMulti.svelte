...
<script lang="ts">
  ...
  let optgroups = {};
  let selectOptionsList = $state();
  let showSelectOptionsList = $state(false);
  let optionsDataType = $state();
...
>
  ...
  <select multiple bind:value={values} onchange={bubble('change')}>
        ...
        {:else}
          {#if optionsDataType === "primitive"}
            {#each values as item, index}
              <span class="value">
                <span class="value-text">{item}</span> <span class="remove-value" onclick={() => removeValue(index)}>&times;</span>
              </span>
            {/each}
          {:else if optionsDataType === "object"}
            {#each values as obj, index}
              <span class="value">
                <span class="value-text">{obj[optionLabel]}</span> <span class="remove-value" onclick={() => removeValue(index)}>&times;</span>
              </span>
            {/each}
          {/if}
        {/if}
