...
<div class="select-wrapper" class:isActive class:disabled>
  <!-- TODO: For some reason the disabled <select> element doesn't style the background color or text color properly. So when this component is disabled, then the <select> element gets replaced with a <div> element for now - at least until I can figure out what is causing that issue. -->
  {#if disabled}
...
<style>
  .select-wrapper {
    ...
    & .select-native {
      ...
      outline-color: var(--custom-select-border-color, var(--border-color-default));
      border-radius: var(--border-radius);
