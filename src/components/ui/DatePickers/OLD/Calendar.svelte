<script lang="ts">
  ...
  let shownDate = $state(value ?? defaultDate);
  ...
  let {
    ...
    max = new Date(defaultDate.getFullYear() + 10, 11, 31),
    locale = {}
  }: Props = $props();


  let year = $state(shownDate.getFullYear());
  ...
  let month = $state(shownDate.getMonth());
  ...
  let dayOfMonth = $state(value?.getDate() || null);
  ...
  let calendarDays = $derived(getCalendarDays(shownDate, iLocale.weekStartsOn));
</script>


<Label {label} forVal={`fp-calendar-${componentId}`} />
<div class="calendar-container" class:popup onfocusout={bubble('focusout')} tabindex="0" onkeydown={keydown}>
  <div class="top">
    <button class="change-month-btn" tabindex="-1" onclick={() => setMonth(month - 1)}>
      &ltrif;
    </button>
    <div class="select-container month">
      <select bind:value={month} onkeydown={monthKeydown}>
    ...
    <div class="select-container year">
      <select bind:value={year} onkeydown={yearKeydown}>
        {#each years as v}
